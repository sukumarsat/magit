  ...
  (if (not range)
    ...
    (when (member "--cover-letter" args)
      (save-match-data
        (find-file
         (expand-file-name
          (concat (seq-some (and (string-match "\\`--reroll-count=\\(.+\\)" %)
                                 (format "v%s-" (match-string 1 %)))
                            args)
                  "0000-cover-letter.patch")
          (let ((topdir (magit-toplevel)))
            (or (seq-some (and (string-match "\\`--output-directory=\\(.+\\)" %)
                               (expand-file-name (match-string 1 %) topdir))
                          args)
                topdir))))))))
